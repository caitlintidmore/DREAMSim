...
CaloTree::CaloTree(string macFileName, int argc, char **argv)
{
   ...
   string outname = getParamS("jobName") + "_run" + getParamS("runNumber") + "_" + getParamS("runSeq") + "_" + getParamS("runConfig") + "_" + getParamS("numberOfEvents") + "evt_" + getParamS("gun_particle") + "_" + getParamS("gun_energy_min") + "_" + getParamS("gun_energy_max");
   ...
   // =====================================
   // histo1D["cerWL"]=new TH1D("cerWL","Cerenkov Wave Length (nm)",100,0.,1000.);
   // histo1D["cerWLelec"]=new TH1D("cerWLelec","Cerenkov Wave Length (had)  (nm)",100,0.,1000.);
   ...
   histo1D["edepS54"] = new TH1D("edepS54", "edep (54x54) (Sci)", nx, xmin, xmax);
   histo1D["edepS54wt1"] = new TH1D("edepS54wt1", "edep (54x54) wt=1 (Sci)", nx, xmin, xmax);

   histo1D["edepS54"] = new TH1D("edepS54", "edep (54x54) (Sci)", nx, xmin, xmax);
   histo1D["edepS54wt1"] = new TH1D("edepS54wt1", "edep (54x54) wt=1 (Sci)", nx, xmin, xmax);

   histo1D["edepC54"] = new TH1D("edepC54", "edep (54x54) (Cher)", nx, xmin, xmax);
   histo1D["edepC54wt1"] = new TH1D("edepC54wt1", "edep (54x54) wt=1 (Cher)", nx, xmin, xmax);

   histo1D["edepRSC54"] = new TH1D("edepRSC54", "edep 54x54 (rod+s+c)", nx, xmin, xmax);

   nx = 200;
   xmin = 0.0;
   xmax = 200.0; //  z-slice max
   histo1D["edepRSCz"] = new TH1D("edepRSCz", "(rod+s+c) edep vs zslice(2cm/slice)", nx, xmin, xmax);
   histo1D["edepCz"] = new TH1D("edepCz", "(c-only) edep vs zslics (2cm/slice)  ", nx, xmin, xmax);

   nx = 200;
   xmin = 0.0;
   xmax = 200.0; // 20000.0 for in photon counts
   histo1D["ncerCsumZ"] = new TH1D("ncerCsumZ", "ncer summed over Z-slices (c)", nx, xmin, xmax);
   histo1D["ncerCsumT"] = new TH1D("ncerCsumT", "ncer summed over T-slicesT (c)", nx, xmin, xmax);
   histo1D["ncerCsumT54"] = new TH1D("ncerCsumT54", "ncer (54x54) summed over T-slicesT (c)", nx, xmin, xmax);
   histo1D["ncerCsumT54wt1"] = new TH1D("ncerCsumT54wt1", "ncer (54x54) (wt1) summed over T-slicesT (c)", nx, xmin, xmax * 100.0);
   ...
   histo2D["ncerCzvsCt"] = new TH2D("ncerCzvsCt", "no of c-photons:  z-slice vs t-slice", 50, 0.0, 100.0, 50, 0.0, 250.);
   ...
   histo2D["ncerIXvsIY"] = new TH2D("ncerIXvsIY", "no of c-photons: x-slice vs y-slice", 30, 0.0, 30.0, 20, 0.0, 20.0);
   histo2D["ncerIXvsIYactive"] = new TH2D("ncerIXvsIYactive", "no of c-photons: x-slice vs y-slice (active)", 30, 0.0, 30.0, 20, 0.0, 20.0);

   histo1D["stepCedepZ"] = new TH1D("stepCedepZ", "stepCedepZ (cm)", 200, -250., 250.);
   histo1D["stepCedepT"] = new TH1D("stepCedepT", "stepCedepT (ns)", 200, 0.0, 100.0);
   histo1D["stepCncerZ"] = new TH1D("stepCncerZ", "stepCedepZ (cm)", 200, -250., 250.);
   histo1D["stepCncerT"] = new TH1D("stepCncerT", "stepCedepT (ns)", 200, 0.0, 100.0);

   histo1D["cerWL"] = new TH1D("cerWL", "wave length (nm)", 200, 0.0, 1000.0);
   histo1D["cerWLcaptured"] = new TH1D("cerWLcaptured", "wave length captured (nm)", 200, 0.0, 1000.0);
   histo1D["cerWLcapturedELEC"] = new TH1D("cerWLcapturedELEC", "wave length capturedElec", 200, 0.0, 1000.0);
   // ==========================
...
CaloTree::~CaloTree()
{
   std::cout << "deleting CaloTree..." << std::endl;
}
...
// ########################################################################
void CaloTree::saveBeamXYZE(string ptype, int pdgid, float x, float y, float z, float en)
{
...
{
   // cout<<"CaloTree::clearCaloTree is called..."<<endl;
...
void CaloTree::accumulateHits(CaloHit ah)
{
   ...
   if (id.type() == 3)
   {
      int t = id.getTkey();
      // t=3;
      ctHits[t] = ctHits[t] + ah.ncer;
      int z = id.getZkey();
      // cout<<"skdebug-  caloTrr accum  z="<<z<<"    iz "<<z/100000<<endl;;
      //  z=13;
      czHits[z] = czHits[z] + ah.ncer;
...
map<int, double> CaloTree::make2Dhits(map<int, double> hits)
{
   ...
   for (auto itr = hits.begin(); itr != hits.end(); itr++)
   {
      // std::cout<<" rt-key "<<itr->first<<"  val "<<itr->second<<std::endl;
      int k = itr->first;
      int mask = (1 << 22) - 1; // assuming bit pattern {2,2,5,5,3,3,2,8} (see CaloID)
      int newkey = k & mask;
...
{
   //  type: "2dSC", "2dCH", "3dCH"
   string csvname = "hit_" + type + "_" + getParamS("runNumber") + "_" + getParamS("runSeq") + "_" + getParamS("runConfig") + "_" + getParamS("numberOfEvents") + "ev_" + getParamS("gun_particle") + "_" + getParamS("gun_energy_min") + "_" + getParamS("gun_energy_max") + "_csv" + getParamS("csvHits" + type) + ".csv";
...
void CaloTree::writeCSV(string type, map<int, double> &hits)
{
   ...
   if (!(fcsv[type]->is_open()))
   {
      cout << "csv file " << type << " is not open.  return without writing." << endl;
   }

   *(fcsv[type]) << eventCountsALL
                 << "," << beamX << "," << beamY << "," << beamZ << "," << beamE << "," << beamType
                 << "," << sum << "," << nhits;
...
void CaloTree::readMacFile(string fileName)
{
   ...
   {
      cout << "CaloTree::readParamFile: error to open mac file, " << fileName << endl;
   }

   cout << " " << endl;
   cout << "=== Parameters from " << fileName << " (CaloTree::readMacFile) ===" << endl;
...
bool CaloTree::setParam(string key, string val)
{
   ...
   if (keyfound)
   {
      std::cout << "CaloTree::setParam: (overwrite)  kew=" << key << "  val=" << val << std::endl;
      mcParams[key] = val;
   }
...
float CaloTree::getParamF(string key)
{
   ...
   {
      std::cout << "  " << std::endl;
      std::cout << "CaloTree::getParamF: Parameter key (" << key << ") does not exist in the mac file. Exit.." << std::endl;
...
int CaloTree::getParamI(string key)
{
   ...
   {
      std::cout << "  " << std::endl;
      std::cout << "CaloTree::getParamI: Parameter key (" << key << ") does not exist in the mac file. Exit.." << std::endl;
      std::cout << "    note:  key word is case sensitive." << std::endl;
